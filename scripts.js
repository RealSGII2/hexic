		...
		for (var i = matrix_h - 1; i >= 0; i--) {
			if (hexmatrix[i][j] < 0) {
				if (tofill == 0) {
					...
					if (Math.floor(Math.random() * 32) == 0 && spawnBombs == true) {
						bombmatrix[i][j] = Math.floor(Math.random() * 35 + 20)
					} else
						bombmatrix[i][j] = -999;
...
function rotate(blocks, ccw) {
	...
	for (var b = 1; b <= blocks.length; b++) {
		...
		blocks[circ_idx].bvalue = bombmatrix[blocks[circ_idx].i][blocks[circ_idx].j];
	}
	return blocks;
}
...
function detect_combs(matrix) {
	...
	if (res_tri.combs || res_circ.combs) {
		console.log("hii")
		bombmatrix = bombmatrix.map(a => a.map(i => {
			if (i < 1 && i > -999) {
				return document.getElementById("losescreen").style.display = "flex";
...
function get_nearest(mouse_x, mouse_y) {
	...
	for (a = 0; a < ijlist.length; a++) {
		...
		center = [blockArea.offsetTop + hex.offsetTop + (hex_h / 2), blockArea.offsetLeft + hex.offsetLeft + (hex_w / 2)];
...
function draw_block(block, isnew) {
	...
	markup += '<div id="' + (isnew ? "new" : "") + 'hex_' + block.i + '_' + block.j + '" class="hexblock" style="' + styles.join("; ") + '">' + inner + '</div>';
	return markup;
}
...
document.querySelectorAll(".replayB").forEach(e => e.addEventListener('click', () => window.location.reload()))